package com.GHTK.Social_Network.security.jwt;
        ...
        try {
          ...
          if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (tokenOptional.isPresent()) {
              ...
              if (!token.isExpired() && !token.isRevoked() && jwtUtils.isTokenValid(jwt, userDetails)) {
                UsernamePasswordAuthenticationToken authenticationToken =
                        new UsernamePasswordAuthenticationToken(userDetails, null, userDetails.getAuthorities());
                authenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
                SecurityContextHolder.getContext().setAuthentication(authenticationToken);
              } else {
                throw new CustomException("Token has expired, revoked, or is invalid", HttpStatus.UNAUTHORIZED);
              }
            } else {
