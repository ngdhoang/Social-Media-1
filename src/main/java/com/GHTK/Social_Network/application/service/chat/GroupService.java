...
//    Member member = groupPort.getMemberByUserId(group.getId(), setMemBerNickNameRequest.getUserId());
  ...
  public List<GroupResponse> getMyGroups(PaginationRequest paginationRequest) {
    ...
    return userGroupInfoList.getUserGroupInfoList().stream()
            .map(userGroup -> {
                      Message message = messagePort.getLastMessageByGroupId(userGroup.getGroupId());
                      String lastMsg = "";
                      if (message != null && userGroup.getLastGroupName() == null && !message.getMsgType().equals(EMessageType.IMAGE)) {
                        lastMsg = message.getContent();
                      }
                      if (message.getMsgType().equals(EMessageType.IMAGE)) {
                        lastMsg = "Image";
                      }
                      return groupInfoToResponse(
                              userGroup,
                              currentUser.getUserId(),
                              lastMsg
                      );
  ...
  private GroupResponse groupInfoToResponse(UserGroup userGroup, Long currentId, String lastMsg) {
    ...
    groupResponse.setLastMessage(lastMsg);
