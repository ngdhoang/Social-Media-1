...
public class AuthService implements AuthPortInput {
  ...
  public AuthResponse refreshToken(String refreshToken, String fingerprinting) {
    ...
    Profile profile = profilePort.takeProfileById(user.getUserId());

    return new AuthResponse(newToken, "", userMapper.userAndProfileToUserDto(user, profile));
  }
  ...
  private AuthResponse authenticationWithDeviceTrust(User user, String fingerprinting) {
    ...
    return new AuthResponse(jwtToken, refreshToken, userMapper.userAndProfileToUserDto(user, profile));
  }
