...
public class UserCollectionAdapter implements UserCollectionPort {
  ...
  @Override
  public UserGroup getUserGroupsByUserId(Long userId) {
  ...
  public void addUserGroup(Long userId, UserGroup newUserGroup) {
    Query query = new Query(Criteria.where("userId").is(userId));
    Update update = new Update().push("userGroupInfoList", userCollectionMapperETD.userGroupToUserGroupInfo(newUserGroup));
