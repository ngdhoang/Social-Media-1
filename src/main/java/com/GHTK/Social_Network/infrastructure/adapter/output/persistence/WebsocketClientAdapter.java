...
//    messageCollection.setCreateAt(Instant.now());
    MessageCollection messageCollection = new MessageCollection(message.getGroupId(), message.getUserAuthId(), null, EMessageTypeCollection.valueOf(message.getMsgType().toString()), message.getContent(), message.getTags(), message.getImages(), new LinkedList<>(), 0L, Instant.now());
