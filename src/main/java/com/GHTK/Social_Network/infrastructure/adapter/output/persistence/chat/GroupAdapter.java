...
public class GroupAdapter implements GroupPort {
  ...
  public Group getGroup(String groupId) {
    if (groupId.contains("_"))
      return getGroupForPersonal(groupId);
    return getGroupForGroup(groupId);
  }
  ...
  public UserCollectionDomain getUserGroups(Long userId, int skip, int limit) {
    ...
    List<AggregationOperation> operations = Arrays.asList(
            ...
            // Unwind the userGroupInfoList array to process each element separately
            Aggregation.unwind("userGroupInfoList"),

            // Sort the unwound documents by lastMsgTime
            Aggregation.sort(Sort.by(Sort.Order.desc("userGroupInfoList.lastMsgTime"))),

            // Skip and limit to implement pagination
            Aggregation.skip((long) skip),
            Aggregation.limit(limit),

            // Group back into the original structure
            Aggregation.group("userId").push("userGroupInfoList").as("userGroupInfoList")
    );
    ...
    UserCollection result = results.getUniqueMappedResult();
  ...
  private void setMemberInUser(Long userReceiveId, String groupName) {
    ...
    userReceive.getUserGroupInfoList().add(
            new UserGroupInfo(groupName, null, null, Instant.now())
    );
  ...
  public Member getLastMember(String groupId) {
    ...
    return memberMapperETD.toDomain(memberCollection);
  }
}
