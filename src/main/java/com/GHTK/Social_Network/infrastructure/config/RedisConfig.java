...
import com.GHTK.Social_Network.infrastructure.payload.dto.*;
...
public class RedisConfig {
  ...
  @Bean
  public RedisTemplate<String, CommentDto> commentDtoRedisTemplate(RedisConnectionFactory redisConnectionFactory) {
    RedisTemplate<String, CommentDto> template = new RedisTemplate<>();
    template.setConnectionFactory(redisConnectionFactory);
    template.setKeySerializer(new StringRedisSerializer());
    template.setValueSerializer(new Jackson2JsonRedisSerializer<>(CommentDto.class));
    return template;
  }
